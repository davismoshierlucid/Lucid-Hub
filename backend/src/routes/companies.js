...
router.post('/companies/:id/snooze', async (req, res, next) => {
  try {
    if (!isUuid(req.params.id)) {
      res.status(400).json({ error: 'Invalid company id' });
      return;
    }
    const raw = req.body?.snooze_until_date;
    if (raw == null || String(raw).trim() === '') {
      res.status(400).json({ error: 'snooze_until_date is required' });
      return;
    }
    const until = new Date(raw);
    if (Number.isNaN(until.getTime())) {
      res.status(400).json({ error: 'Invalid snooze_until_date' });
      return;
    }
    if (until.getTime() <= Date.now()) {
      res.status(400).json({
        error: 'snooze_until_date must be in the future',
      });
      return;
    }
    ...
    const { rows } = await pool.query(
      ...
      WHERE id = $2
      RETURNING *`,
      [until.toISOString(), req.params.id]
    );
    if (!rows.length) {
      res.status(404).json({ error: 'Company not found' });
      return;
